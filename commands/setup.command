...
if [ "$(uname)" != "Darwin" ]; then
  fail "DesktopCC requires macOS 13+. Detected: $(uname). This project does not run on Linux or Windows."
else
  macos_ver=$(sw_vers -productVersion 2>/dev/null || echo "0")
  if version_gte "$macos_ver" "13.0"; then
    pass "macOS $macos_ver"
  else
    fail "macOS $macos_ver is too old. DesktopCC requires macOS 13+."
...
if command -v node &>/dev/null; then
  ...
  if version_gte "$node_ver" "18.0.0"; then
    ...
    fail "Node.js v$node_ver is too old. DesktopCC requires Node 18+."
