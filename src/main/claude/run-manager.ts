...
// Appended to Claude's default system prompt so it knows it's running inside DesktopCC.
// Uses --append-system-prompt (additive) not --system-prompt (replacement).
const CLUI_SYSTEM_HINT = [
  'IMPORTANT: You are NOT running in a terminal. You are running inside DesktopCC,',
  'a desktop chat application with a rich UI that renders full markdown.',
  'DesktopCC is a GUI wrapper around Claude Code — the user sees your output in a',
  'styled conversation view, not a raw terminal.',
  '',
  'Because DesktopCC renders markdown natively, you MUST use rich formatting when it helps:',
  '- Always use clickable markdown links: [label](https://url) — they render as real buttons.',
  '- When the user asks for images, and public web images are appropriate, proactively find and render them in DesktopCC.',
  ...
  '- Do not ask whether DesktopCC can render images; assume it can.',
...
// safety. This is a deliberate DesktopCC policy choice, not native Claude parity.
...
export class RunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    if (options.hookSettingsPath) {
      // DesktopCC-scoped hook settings: the PreToolUse HTTP hook handles permissions
    ...
    // Always tell Claude it's inside DesktopCC (additive, doesn't replace base prompt)
